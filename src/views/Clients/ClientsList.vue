<template>
  <v-container fill-height fluid grid-list-xl class="pt-0">
    <!-- CLIENT INFO DIALOG -->
    <v-dialog v-model="clientInfoDialog" max-width="80vw">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-tooltip top>
            <v-btn class="mb-2" slot="activator" fab icon round color="blue">
              <v-icon>mdi-chart-bar</v-icon>
            </v-btn>
            <span>See Analytics</span>
          </v-tooltip>
          <v-tooltip top>
            <v-btn class="ml-2 mb-2" slot="activator" fab icon round color="indigo">
              <v-icon>mdi-file</v-icon>
            </v-btn>
            <span>Create Report</span>
          </v-tooltip>
          <v-tooltip top>
            <v-btn class="ml-2 mb-2" slot="activator" fab icon round color="info">
              <v-icon>mdi-lead-pencil</v-icon>
            </v-btn>
            <span>Edit</span>
          </v-tooltip>
          <v-tooltip top>
            <v-btn class="ml-2 mb-2" slot="activator" fab icon round color="danger">
    ...
    <!-- FILTER DIALOG -->
    <v-dialog v-model="filterDialog" max-width="40vw">
      <v-card>
        ...
        <v-layout row wrap class="justify-center">
          ...
          <v-flex shrink>
            <v-btn class="ml-2" color='grey'>Sleeping</v-btn>
          </v-flex>
          <v-flex shrink>
            <v-btn class="ml-2" color='grey'>Recent</v-btn>
          </v-flex>
          <v-flex shrink>
            <v-btn class="ml-2" color='grey'>2020</v-btn>
          </v-flex>
        </v-layout>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn @click="filterDialog = false" flat="flat" color="red darken-1">Close</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- CLIENT LIST START -->
    <v-layout justify-center wrap>
      <v-flex>
        ...
        <v-layout row wrap class="justify-space-between">
          <v-flex>
            <v-btn color="green mr-3" href="/add-client">
              <v-icon left>mdi-plus</v-icon>Add new
            </v-btn>
            <v-btn v-on:click.stop="openFilterDialog" color="blue darken-4">
              <v-icon left>mdi-filter</v-icon>FILTER
            </v-btn>
            <v-btn small round color="grey" class="ml-2">
              active
        ...
        <material-card>
          ...
          <v-data-table :headers="headers" :items="items" hide-actions :search="search">
            ...
            <template slot="items" slot-scope="{ item }">
              <td v-on:click.stop="openClienInfoDialog(item)" class="clickable">{{ item.name }}</td>
              <td class="hidden-sm-and-down">{{ item.id }}</td>
              <td class="hidden-sm-and-down">{{ item.ae }}</td>
              <td class="hidden-sm-and-down">{{ item.ac }}</td>
              <td class="text-xs-right">
                <v-tooltip top>
                  <v-btn slot="activator" fab icon round color="blue darken-4">
                    <v-icon>mdi-chart-bar</v-icon>
                  </v-btn>
                  <span>See Analytics</span>
                </v-tooltip>
                <v-tooltip top>
                  <v-btn slot="activator" fab icon round color="blue darken-2">
                    <v-icon>mdi-file</v-icon>
                  </v-btn>
                  <span>Create Report</span>
                </v-tooltip>
                <v-tooltip top>
                  <v-btn slot="activator" fab icon round color="blue">
                    <v-icon>mdi-lead-pencil</v-icon>
                  </v-btn>
                  <span>Edit</span>
                </v-tooltip>
                <v-tooltip top>
                  <v-btn slot="activator" fab icon round color="danger">
                    <v-icon>mdi-cancel</v-icon>
                  </v-btn>
                  <span>Deactivate</span>
                </v-tooltip>
